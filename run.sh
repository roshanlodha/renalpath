...
./env/bin/python main.py --mode train --model_type resnet --epochs 10 --batch_size 32
if [ $? -ne 0 ]; then
...
./env/bin/python main.py --mode train --model_type vit --epochs 10 --batch_size 32
if [ $? -ne 0 ]; then
...
if [ -f "$GSVIT_PKL" ]; then
    ./env/bin/python main.py --mode train --model_type gsvit --epochs 10 --batch_size 32 --gsvit_path "$GSVIT_PKL"
    if [ $? -ne 0 ]; then
